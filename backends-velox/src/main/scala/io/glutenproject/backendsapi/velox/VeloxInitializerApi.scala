...
//      .loadAndCreateLink("libarrow.800.dylib", "libarrow.800.dylib", false)
//      .loadAndCreateLink("libarrow.800.dylib", "libarrow.800.dylib", false)
//      .commit()
...
//    }
    ...
    if (StringUtils.isNotBlank(libPath)) { // Path based load. Ignore all other loadees.
      ...
      file.listFiles()
        .filter(_.getName.contains("lib"))
        .foreach(file => JniLibLoader.loadFromPath(libPath + "/" + file.getName, true))
      return
    }
